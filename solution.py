...
import math # need to add to requirements txt?
        ...
        for _ in progress_bar:
            ...
            for batch_idx, (batch_x, batch_y) in enumerate(train_loader):
                ...
                if isinstance(self.network, DenseNet):
                    ...
                    negative_log_likelihood = F.nll_loss(F.log_softmax(current_logits, dim=1), batch_y, reduction='sum')
...
class BayesianLayer(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        self.prior = ScaleMixtureGaussian(torch.tensor(0.5), torch.tensor(1), torch.tensor(0.1))
...
class BayesNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> typing.Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        for idx, current_layer in enumerate(self.layers):
            new_features, log_prior_i, log_variational_posterior_i = current_layer(current_features)
...
class UnivariateGaussian(ParameterDistribution):
    ...
    def __init__(self, mu: torch.Tensor, sigma: torch.Tensor):
        ...
        self.sigma = sigma.to(torch.float32)

    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        # TODO: Implement this
        normalisation_constant = 1/torch.sqrt(torch.tensor(2)*torch.pi*self.sigma.square())
        return torch.sum(torch.log(normalisation_constant * torch.exp(-0.5*torch.square(values - self.mu)/torch.square(self.sigma))))
...
class MultivariateDiagonalGaussian(ParameterDistribution):
    ...
    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        ...
        Sigma = torch.diag(sigma_square)
        normalisation_constant = 1 / ((2 * torch.pi)**(values.shape[0]/2) * Sigma.det().sqrt())
        log_likelihood = torch.log(normalisation_constant * torch.exp(-0.5*(values-self.mu).matmul(Sigma.inverse()).matmul((values-self.mu).transpose(-1,0))))
    ...
    def sample(self) -> torch.Tensor:
        ...
        m = torch.nn.Softplus()
        sigma = m(self.rho)
        return torch.normal(mean = self.mu, std = sigma)
